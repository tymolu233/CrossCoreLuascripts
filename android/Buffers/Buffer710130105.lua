...
---------------------------------------------
-- 技能基类
Buffer710130105 = oo.class(BuffBase)
function Buffer710130105:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 伤害前
function Buffer710130105:OnBefourHurt(caster, target)
	-- 8065
	...
	if self:Rand(c787*1000+5000) then
		self:AddTempAttr(BufferEffect[710130110], self.caster, self.caster, nil, "damage",0.15*self.nCount)
	end
end
-- 创建时
function Buffer710130105:OnCreate(caster, target)
	-- 710130105
	self:AddAttr(BufferEffect[710130105], self.caster, self.card, nil, "damage",0.15*self.nCount)
end
