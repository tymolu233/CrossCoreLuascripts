...
---------------------------------------------
-- 技能基类
Buffer710130101 = oo.class(BuffBase)
function Buffer710130101:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 伤害前
function Buffer710130101:OnBefourHurt(caster, target)
	-- 8065
	...
	if self:Rand(c787*1000+5000) then
		self:AddTempAttr(BufferEffect[710130106], self.caster, self.caster, nil, "damage",0.05*self.nCount)
	end
end
-- 创建时
function Buffer710130101:OnCreate(caster, target)
	-- 710130101
	self:AddAttr(BufferEffect[710130101], self.caster, self.card, nil, "damage",0.05*self.nCount)
end
