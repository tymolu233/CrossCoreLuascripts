...
---------------------------------------------
-- 技能基类
Buffer710230204 = oo.class(BuffBase)
function Buffer710230204:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 行动开始
function Buffer710230204:OnActionBegin(caster, target)
	-- 8160
...
-- 伤害前
function Buffer710230204:OnBefourHurt(caster, target)
	-- 710230212
	self:tFunc_710230212_710230213(caster, target)
	self:tFunc_710230212_710230214(caster, target)
end
-- 创建时
function Buffer710230204:OnCreate(caster, target)
	-- 710230204
	self:AddAttrPercent(BufferEffect[710230204], self.caster, self.card, nil, "attack",0.2)
	-- 710230209
	self:AddAttrPercent(BufferEffect[710230209], self.caster, self.creater, nil, "attack",0.2)
end
function Buffer710230204:tFunc_710230212_710230213(caster, target)
	-- 8060
...
function Buffer710230204:tFunc_710230212_710230214(caster, target)
	-- 8160
	if SkillJudger:IsCasterBuff(self, self.caster, target, true) then
	else
		return
