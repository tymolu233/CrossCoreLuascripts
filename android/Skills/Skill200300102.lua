...
---------------------------------------------
-- 技能基类
Skill200300102 = oo.class(SkillBase)
function Skill200300102:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200300102:DoSkill(caster, target, data)
	-- 12001
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12001], caster, target, data, 1,1)
end
-- 行动结束
function Skill200300102:OnActionOver(caster, target, data)
	-- 8781
	...
	if SkillJudger:Greater(self, caster, target, true,count781,0) then
		...
		for i,target in ipairs(targets) do
			self:AddBuff(SkillEffect[200300113], caster, target, data, 200300107)
		end
	else
		-- 8200
		if SkillJudger:IsCurrSkill(self, caster, target, true) then
		...
		local targets = SkillFilter:MaxAttr(self, caster, target, 1,"attack",1)
		for i,target in ipairs(targets) do
			self:AddBuff(SkillEffect[200300108], caster, target, data, 200300102)
		end
		-- 8777
...
-- 行动结束2
function Skill200300102:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 行动开始
function Skill200300102:OnActionBegin(caster, target, data)
	-- 8778
