...
---------------------------------------------
-- 技能基类
Skill200300202 = oo.class(SkillBase)
function Skill200300202:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200300202:DoSkill(caster, target, data)
	-- 200300215
	self.order = self.order + 1
	self:AddBuff(SkillEffect[200300215], caster, target, data, 200300202)
end
-- 行动开始
function Skill200300202:OnActionBegin(caster, target, data)
	-- 8200
...
-- 行动结束2
function Skill200300202:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 行动结束
function Skill200300202:OnActionOver(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
	...
	for i,target in ipairs(targets) do
		...
		if SkillJudger:Greater(self, caster, target, true,count780,0) then
			-- 200300220
			self:AddBuff(SkillEffect[200300220], caster, target, data, 200300207)
		else
			-- 8777
