...
---------------------------------------------
-- 技能基类
Skill200301301 = oo.class(SkillBase)
function Skill200301301:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200301301:DoSkill(caster, target, data)
	-- 12003
	...
	for i,target in ipairs(targets) do
		self:AddBuff(SkillEffect[200300301], caster, target, data, 200300301)
	end
end
-- 行动开始
function Skill200301301:OnActionBegin(caster, target, data)
	-- 8200
...
-- 行动结束
function Skill200301301:OnActionOver(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
	...
	for i,target in ipairs(targets) do
		...
		if SkillJudger:Greater(self, caster, target, true,count780,0) then
			-- 200300316
			self:AddBuff(SkillEffect[200300316], caster, target, data, 200300306)
		else
			-- 8777
...
-- 行动结束2
function Skill200301301:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 攻击开始
function Skill200301301:OnAttackBegin(caster, target, data)
	-- 8777
	local count777 = SkillApi:SkillLevel(self, caster, target,3,42003)
	-- 8782
	...
	-- 200301303
	self:AddBuffCount(SkillEffect[200301303], caster, target, data, 4200306,count782,10)
	-- 200301308
