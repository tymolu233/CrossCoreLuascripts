...
---------------------------------------------
-- 技能基类
Skill200300104 = oo.class(SkillBase)
function Skill200300104:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200300104:DoSkill(caster, target, data)
	-- 12001
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12001], caster, target, data, 1,1)
end
-- 行动结束
function Skill200300104:OnActionOver(caster, target, data)
	-- 8781
	...
	if SkillJudger:Greater(self, caster, target, true,count781,0) then
		...
		for i,target in ipairs(targets) do
			self:AddBuff(SkillEffect[200300115], caster, target, data, 200300109)
		end
	else
		-- 8200
		if SkillJudger:IsCurrSkill(self, caster, target, true) then
		...
		local targets = SkillFilter:MaxAttr(self, caster, target, 1,"attack",1)
		for i,target in ipairs(targets) do
			self:AddBuff(SkillEffect[200300110], caster, target, data, 200300104)
		end
		-- 8777
...
-- 行动结束2
function Skill200300104:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 行动开始
function Skill200300104:OnActionBegin(caster, target, data)
	-- 8778
