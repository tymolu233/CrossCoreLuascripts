...
---------------------------------------------
-- 技能基类
Skill200301304 = oo.class(SkillBase)
function Skill200301304:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200301304:DoSkill(caster, target, data)
	-- 12003
	...
	for i,target in ipairs(targets) do
		self:AddBuff(SkillEffect[200300304], caster, target, data, 200300304)
	end
end
-- 行动开始
function Skill200301304:OnActionBegin(caster, target, data)
	-- 8200
...
-- 行动结束
function Skill200301304:OnActionOver(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
	...
	for i,target in ipairs(targets) do
		...
		if SkillJudger:Greater(self, caster, target, true,count780,0) then
			-- 200300319
			self:AddBuff(SkillEffect[200300319], caster, target, data, 200300309)
		else
			-- 8777
...
-- 行动结束2
function Skill200301304:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 攻击开始
function Skill200301304:OnAttackBegin(caster, target, data)
	-- 8777
	local count777 = SkillApi:SkillLevel(self, caster, target,3,42003)
	-- 8782
	...
	-- 200301306
	self:AddBuffCount(SkillEffect[200301306], caster, target, data, 4200309,count782,10)
	-- 200301308
