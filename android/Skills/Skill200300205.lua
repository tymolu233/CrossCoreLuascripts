...
---------------------------------------------
-- 技能基类
Skill200300205 = oo.class(SkillBase)
function Skill200300205:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200300205:DoSkill(caster, target, data)
	-- 200300218
	self.order = self.order + 1
	self:AddBuff(SkillEffect[200300218], caster, target, data, 200300205)
end
-- 行动开始
function Skill200300205:OnActionBegin(caster, target, data)
	-- 8200
...
-- 行动结束2
function Skill200300205:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 行动结束
function Skill200300205:OnActionOver(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
	...
	for i,target in ipairs(targets) do
		...
		if SkillJudger:Greater(self, caster, target, true,count780,0) then
			-- 200300223
			self:AddBuff(SkillEffect[200300223], caster, target, data, 200300210)
		else
			-- 8777
