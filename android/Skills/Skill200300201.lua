...
---------------------------------------------
-- 技能基类
Skill200300201 = oo.class(SkillBase)
function Skill200300201:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill200300201:DoSkill(caster, target, data)
	-- 200300214
	self.order = self.order + 1
	self:AddBuff(SkillEffect[200300214], caster, target, data, 200300201)
end
-- 行动开始
function Skill200300201:OnActionBegin(caster, target, data)
	-- 8200
...
-- 行动结束2
function Skill200300201:OnActionOver2(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
...
-- 行动结束
function Skill200300201:OnActionOver(caster, target, data)
	-- 8200
	if SkillJudger:IsCurrSkill(self, caster, target, true) then
	else
		return
	...
	for i,target in ipairs(targets) do
		...
		if SkillJudger:Greater(self, caster, target, true,count780,0) then
			-- 200300219
			self:AddBuff(SkillEffect[200300219], caster, target, data, 200300206)
		else
			-- 8777
