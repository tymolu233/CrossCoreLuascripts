...
function Refresh(_data)
    ...
    val3Color=_data and _data.lockColor or "c3c3c8"
    ...
    if data.lockStr then
        str3 = StringUtil:SetByColor(data.lockStr, val3Color)
    end
    -- name
    SetName(str1);
    SetNum(str2);
    SetLv(str3);
    SetLock(data.isLock);
    CSAPI.SetGOAlpha(node,data.isLock and 0.5 or 1);
