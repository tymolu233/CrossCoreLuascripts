...
function InitTab()
    if data.spType==1 or data.spType==2 then
        ...
        CSAPI.SetGOActive(item2, cfg.price_1 ~= nil)
        if (cfg.price_1 ~= -1) then
            local _cfg1 = Cfgs.ItemInfo:GetByID(cfg.price_1[1][1])
            ResUtil.IconGoods:Load(tIcon1, _cfg1.icon .. "_1", true)
            ResUtil.IconGoods:Load(tsIcon1,_cfg1.icon .. "_1",true);
            LanguageMgr:SetText(txtNormal1, 18111, _cfg1.name) -- 38001
        ...
        if (cfg.price_2 ~= -1) then
            local _cfg2 = Cfgs.ItemInfo:GetByID(cfg.price_2[1][1])
            ResUtil.IconGoods:Load(tIcon2, _cfg2.icon .. "_1", true)
            ResUtil.IconGoods:Load(tsIcon2,_cfg2.icon .. "_1",true);
            LanguageMgr:SetText(txtNormal2, 18111, _cfg2.name) -- 38002
            LanguageMgr:SetText(txtSel2, 18111, _cfg2.name)
        else
    ...
    elseif data.spType==3 then
        ...
        if costs then
            ...
            if costs[2].id~=-1 then
                local _cfg2 = Cfgs.ItemInfo:GetByID(costs[2].id)
                ResUtil.IconGoods:Load(tIcon2, _cfg2.icon .. "_1", true)
                ResUtil.IconGoods:Load(tsIcon2,_cfg2.icon .. "_1",true);
                LanguageMgr:SetText(txtNormal2, 18111, _cfg2.name) -- 38002
                LanguageMgr:SetText(txtSel2, 18111, _cfg2.name)
...
function InitContent()
    ...
    RefreshPriceInfo();
    RefreshVoucherItem();
    ItemUtil.AddItems(path, curList, curDatas, Content, nil, 1,costIdx)
end
...
function RefreshVoucherItem()
	local isShow=false;
    if data.spType==nil and voucherItem~=nil and comm and comm:GetUseVoucherTypes()~=nil then
...
function InitPrice()
    ...
    elseif comm then
        priceInfo=comm:GetRealPrice(shopPriceKey);
...
function InitNumObj()
    ...
    local limitNum=0;
    local priceInfo=nil;
    if data.spType==1 or (comm and comm.GetType~=nil and comm:GetType()==CommodityItemType.FORNITURE and data.spType==nil) then --家具
        maxNum = data.cfg.buyNumLimit
        local cur = DormMgr:GetBuyCount(data.cfg.id);
        if comm then
            limitNum=comm:GetNum();
            limitNum=cur+limitNum>=maxNum and maxNum-cur or limitNum;
            priceInfo=comm:GetRealPrice(shopPriceKey);
            priceInfo=priceInfo and priceInfo[1] or nil;
        else
    ...
    CSAPI.SetGOActive(numObj,maxNum>1)
end
...
function OnClickPay()
    if comm and data.spType==nil then
        ...
        elseif comm:GetType() == CommodityItemType.DoubleSelection  then
            if (curIdx2 <= 0 or curIdx1 < 0) then --自选未选择
                return
            end
            comm:SetGrid2(curDatas[2].itemData:GetID())
