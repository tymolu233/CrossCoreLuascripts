...
function SetProgress()
    ...
    if info then
        ...
        local cfgReward = Cfgs.CfgAnniversaryStarReward:GetByID(info[2])
...
function SetLItems()
    items1 = items1 or {}
    ItemUtil.AddItems("Anniversary4/AnniversaryMain3Item",items1,mCfgs,itemParent1,nil,1,score,OnLoadSuccess)
end
...
function SetRItems()
    items2 = items2 or {}
    ItemUtil.AddItems("Anniversary4/AnniversaryMain3Item2",items2,rDatas,itemParent2,OnItemClickCB,1,{group = data:GetGroup()})
end
...
function SetSvWidth()
    local w1 = 2094 + (#rDatas * (46 + 358)) + 200
    CSAPI.SetRTSize(content,w1,0)
    CSAPI.SetRTSize(imgLen, w1 - 311,16)
