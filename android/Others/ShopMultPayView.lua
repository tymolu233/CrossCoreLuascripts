...
function RefreshVoucherItem()
    ...
    if voucherItem~=nil and commodity and commodity:GetUseVoucherTypes()~=nil then
        voucherItem.Init(commodity,currNum,true,shopPriceKey);
...
function OnVoucherChange(ls)
    ...
    if voucherList~=nil then
        realPrice=0;
        local isOk,tips,res=GLogicCheck:IsCanUseVoucher(commodity:GetCfg(),voucherList,TimeUtil:GetTime(),currNum,PlayerClient:GetLv(),false,shopPriceKey);
