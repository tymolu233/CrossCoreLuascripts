-- 头像框（只处理红点）
local this = MgrRegister("MenuThemeMgr")
...
function this:UpdateGoodsData(goodsData, setNew)
    ...
    if (isChange) then
        RedPointMgr:UpdateData(RedPointType.MenuTheme, self:IsRed())
    end
...
function this:CheckRed(id)
    if (id and self.datas[id] and self.datas[id][1] == 1) then
        return true
    else
        return false
    end
...
-- 主界面样式
function this:GetMenuThemeID()
    return self.themeId or 1
end
function this:SetMenuThemeID(_themeId)
    self.themeId = _themeId
end

-- 临时样式
function this:GetTempMenuThemeID()
    return self.teampMTID or self:GetMenuThemeID()
end

function this:SetTempMenuThemeID(id)
    self.teampMTID = id
end

-- 主题是否过期
function this:CheckMenuThemeEnd()
    local useID = MenuThemeMgr:GetTempMenuThemeID()
    if (useID == 1) then
        return
    end
    local isEnd = false
    local cfg = Cfgs.CfgUiTheme:GetByID(useID)
    local goodsData = BagMgr:GetData(cfg.item_id)
    if (goodsData == nil or goodsData:GetCount() < 1 or
