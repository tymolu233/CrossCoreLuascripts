...
function OnOpen()
    if data then
        ...
        if voucherItem~=nil then
            voucherItem.Init(data,1,false,openSetting);
...
function OnVoucherChange(ls)
    ...
    if voucherList~=nil then
        realPrice=0;
        local isOk,tips,res=GLogicCheck:IsCanUseVoucher(data:GetCfg(),voucherList,TimeUtil:GetTime(),1,PlayerClient:GetLv(),false,openSetting);
