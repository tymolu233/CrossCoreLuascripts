...
function Refresh()
	...
	if commodity then
		...
		--用于显示限定时间（旧商店UI）
		if g_EternityWarzoneShop and commodity:GetID() == g_EternityWarzoneShop then
			local resetTime = TimeUtil:NextWeekIndexTimeStamp(1, g_ActivityDiffDayTime)
			local tab=TimeUtil:GetDiffHMS(resetTime,TimeUtil.GetTime());
			LanguageMgr:SetText(txt_limitTimeTips,312024)
			if tab.day>0 then
                LanguageMgr:SetText(txt_limitTime,60005,tab.day)
            elseif tab.hour>0 then
                LanguageMgr:SetText(txt_limitTime,60009,tab.hour)
            else
                if tab.minute>=1 then
                    LanguageMgr:SetText(txt_limitTime,60010,tab.minute)
                else
                    LanguageMgr:SetText(txt_limitTime,60010,"0")
                end
            end
			CSAPI.SetGOActive(limitTimeObj,resetTime - TimeUtil:GetTime() > 0);
		end
...
function OnClickPay()
	--(旧UI逻辑)
	if g_EternityWarzoneShop and commodity:GetID() == g_EternityWarzoneShop and commodity:GetNum() == 0 then
		LanguageMgr:ShowTips(75003)
		return
	end
